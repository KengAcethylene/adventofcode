...
const timesMap = times.slice(times.indexOf(":")+1).trim().split(/\s+/).map((x)=>parseInt(x));
const distancesMap = distances.slice(distances.indexOf(":")+1).trim().split(/\s+/).map((x)=>parseInt(x));
let answer = 1;
