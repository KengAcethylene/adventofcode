...
let idx = 0, step = 0, destination = Object.keys(map).filter((key)=> key.endsWith('A'));
let answer = (new Array(destination.length)).fill(0).map((x) => {return {amount:0, step:0}});

let limit = 2;
while(!answer.every((step)=>(step.step === limit))){
    idx = idx === instructions.length ? 0: idx;
    for (const [desIdx, key] of destination.entries()){
        destination[desIdx] = instructions[idx] === 'L' ? map[key][0] : map[key][1];
